...
const tweets = () => {
    ...
    const handleSubmit =async (e)=>{
        ...
        setHeading(`The tweet for  ${data.productname} is:`)
        const configuration = new Configuration({
        apiKey: process.env.OPENAI_API_KEY,
        });
        const openai = new OpenAIApi(configuration);
        console.log(`Write detailed,smart and informative tweet on ${data.productname}`);
        const response = await openai.createCompletion({
        model: "text-davinci-003",
        prompt: `Write detailed,smart and informative tweet on ${data.productname}`,
        temperature: 0.92,
        max_tokens: 256,
        top_p: 1,
        frequency_penalty: 0,
